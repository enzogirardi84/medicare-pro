...
# --- CARGA INICIAL (EVITA QUE SE BORREN LOS USUARIOS) ---
...
with tabs[0]:
    ...
    with col_der:
        ...
        if paciente_sel:
            st.warning(f"¿Estás seguro de que querés borrar a: {paciente_sel}?")
            confirmar = st.checkbox("Confirmar eliminación (Esta acción no se puede deshacer)")
            if st.button("🔴 Eliminar Paciente Definitivamente") and confirmar:
                ...
                # Limpiamos registros relacionados (opcional pero recomendado)
                st.session_state["evoluciones_db"] = [e for e in st.session_state["evoluciones_db"] if e["paciente"] != paciente_sel]
                st.session_state["vitales_db"] = [v for v in st.session_state["vitales_db"] if v["paciente"] != paciente_sel]
                st.session_state["indicaciones_db"] = [i for i in st.session_state["indicaciones_db"] if i["paciente"] != paciente_sel]
                
                guardar_datos()
                st.success("Paciente eliminado correctamente.")
                st.rerun()
        else:
            st.info("Seleccioná un paciente en la barra lateral para poder eliminarlo.")
...
with tabs[2]:
    if paciente_sel:
        ...
        for e in reversed([x for x in st.session_state["evoluciones_db"] if x["paciente"] == paciente_sel]):
            st.write(f"**{e['fecha']}** | {e['nota']} (Firma: {e['firma']})")
...
with tabs[3]:
    if paciente_sel:
        with st.form("recet"):
            ...
            drog = c1.text_input("Droga"); dos = c2.text_input("Dosis")
            c3, c4, c5 = st.columns(3)
            via = c3.selectbox("Vía", ["Oral", "Endovenosa", "Intramuscular", "Subcutánea"])
            frec = c4.selectbox("Frecuencia", ["Cada 4hs", "Cada 6hs", "Cada 8hs", "Cada 12hs", "Cada 24hs", "Dosis Única", "S.O.S"])
    ...
    if paciente_sel and FPDF_DISPONIBLE:
        def crear_pdf(p):
            ...
            for ev in [e for e in st.session_state["evoluciones_db"] if e["paciente"] == p]:
                pdf.multi_cell(0, 6, txt(f"[{ev['fecha']}] {ev['nota']} (Firma: {ev['firma']})")); pdf.ln(2)
        ...
        if st.button("Bajar PDF"):
            st.download_button("Descargar", crear_pdf(paciente_sel), f"HC_{paciente_sel}.pdf", "application/pdf")

    ...
    if db:
        ...
        for c in claves_iniciales:
            if c not in st.session_state:
                if c == "usuarios_db": 
                    st.session_state[c] = {"admin": {"pass": "admin123", "rol": "SuperAdmin", "nombre": "Enzo Girardi", "empresa": "SISTEMAS E.G.", "dni": "3584302024", "matricula": "M.P 21947", "titulo": "Director de Sistemas"}}
...
if not st.session_state["logeado"]:
    ...
    with col:
        ...
        with st.form("login"):
            u = st.text_input("Usuario (ej: admin)")
            p = st.text_input("Contraseña (ej: admin123)", type="password")
            if st.form_submit_button("Ingresar al Sistema", use_container_width=True):
                # Forzamos lectura para no perder usuarios recién creados
...
with st.sidebar:
    ...
    if paciente_sel:
        ...
        st.caption(f"**Nacimiento:** {detalles_sidebar.get('fnac', 'S/D')}")
...
# 1. ADMISIÓN (COMPLETA Y CON BORRADO)
with tabs[0]:
    c1, c2 = st.columns([2, 1])
    with c1:
        st.subheader(f"Registrar Paciente")
        with st.form("adm_form", clear_on_submit=True):
            ...
            ant = st.text_area("Antecedentes Médicos (Alergias, Cirugías, Enfermedades Crónicas)")
                ...
                if n and d:
                    ...
                    if id_p not in st.session_state["pacientes_db"]:
                        ...
                        st.session_state["detalles_pacientes_db"][id_p] = {
                            "dni": d, "fnac": f_nac.strftime("%d/%m/%Y"), 
                            "antecedentes": ant, "empresa": mi_empresa
                        }
    ...
    with c2:
        ...
        if paciente_sel:
            st.warning(f"¿Borrar permanentemente a {paciente_sel}?")
...
with tabs[1]:
    if paciente_sel:
        ...
        if detalles_pac.get("antecedentes"):
            st.error(f"**⚠️ Antecedentes Médicos:** {detalles_pac.get('antecedentes')}")
        ...
        with st.form("vitales_f"):
            ...
            if sat < 90 or fc > 130: st.warning("Prioridad: 🔴 EMERGENCIA")
            elif sat < 94: st.warning("Prioridad: 🟡 URGENCIA")
...
with tabs[3]:
    if paciente_sel:
        ...
        with st.form("recet_pro"):
            ...
            drog = col1.text_input("Medicamento"); dos = col2.text_input("Dosis (ej: 500mg, 1 amp)")
            c3, c4, c5 = st.columns(3)
            via = c3.selectbox("Vía", ["Oral", "Endovenosa", "Intramuscular", "Subcutánea", "Inhalatoria", "Tópica"])
            frec = c4.selectbox("Horario / Frecuencia", ["Cada 2 hs", "Cada 4 hs", "Cada 6 hs", "Cada 8 hs", "Cada 12 hs", "Cada 24 hs", "Dosis Única", "S.O.S (Si dolor)"])
...
# 6. PDF (REPORTES COMPLETOS LEGALES)
with tabs[5]:
    if paciente_sel and FPDF_DISPONIBLE:
        def crear_pdf_final(p):
            ...
            if det.get('antecedentes'):
                pdf.set_font("Arial", 'B', 10); pdf.cell(0, 6, t("ANTECEDENTES MÉDICOS:"), ln=True)
                pdf.set_font("Arial", '', 10); pdf.multi_cell(0, 6, t(det.get('antecedentes')))
            pdf.line(10, pdf.get_y()+2, 200, pdf.get_y()+2); pdf.ln(8)
            
            pdf.set_font("Arial", 'B', 12); pdf.cell(0, 8, t("1. EVOLUCIONES CLINICAS:"), ln=True)
            pdf.set_font("Arial", '', 10)
            ...
            pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.cell(0, 8, t("2. PLAN TERAPEUTICO / RECETARIO:"), ln=True)
            pdf.set_font("Arial", '', 10)
        ...
        if st.button("📥 Descargar Reporte Completo PDF"):
            st.download_button("Bajar Archivo", crear_pdf_final(paciente_sel), f"HC_{paciente_sel}.pdf", "application/pdf")

# 7. EQUIPO (GESTIÓN DE PERSONAL)
with tabs[6]:
    ...
    with st.form("new_staff"):
        ...
        if st.form_submit_button("Habilitar Profesional"):
            ...
            guardar_datos(); st.success("¡Usuario creado correctamente!"); st.rerun()

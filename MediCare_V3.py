...
if "theme" not in st.session_state:
    st.session_state.theme = "light"   # Modo por defecto: Claro (profesional médico)

# Botón premium en el sidebar
with st.sidebar:
    ...
    with col_t2:
        if st.button(
            "☀️ Claro" if st.session_state.theme == "dark" else "🌙 Oscuro",
...
# ====================== DISEÑO VISUAL ENTERPRISE PREMIUM v5.0 ======================
page_bg_css = f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');

:root {{
    /* === COLORES REFINADOS PARA MODO CLARO Y OSCURO === */
    --bg-app: {('#f8fafc' if st.session_state.theme == 'light' else '#0a0a12')};
    --bg-card: {('rgba(255,255,255,0.97)' if st.session_state.theme == 'light' else 'rgba(20,20,35,0.95)')};
    ...
    --shadow-base: {('0 8px 25px -6px rgb(0 0 0 / 0.07)' if st.session_state.theme == 'light' else '0 12px 35px -8px rgb(0 0 0 / 0.5)')};
